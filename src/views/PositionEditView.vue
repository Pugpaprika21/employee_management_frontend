...
const getPositonById = () => {
  axios
    .get(
      import.meta.env.VITE_API_ENDPOINT + "getPositionById/" + route.params.id
    )
    .then((res) => {
      if (res.data.status_bool) {
        const positonEditData = res.data.data;
        positon.value.name = positonEditData.name;
        positon.value.description = positonEditData.description;
        positon.value.salary = positonEditData.salary;
        positon.value.responsibilities = positonEditData.responsibilities;
        positon.value.location = positonEditData.location;
        return;
      }
    })
    .catch((err) => {
...
<template>
  <div class="main container mt-4">
    ...
    <div class="emp_form_save">
      <div class="card">
        <div class="card-body">
          <form @submit.prevent="onUpdatePositonById()">
            <div class="row mb-3">
              <div class="col-md-6">
